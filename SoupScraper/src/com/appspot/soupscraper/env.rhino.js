...
(function($env){
    ...
    $env.log = function(msg, level){
         print(' '+ (level?level:'LOG') + ':\t['+ new Date() +" "+ (new Date().getTime()-$env.startTime) +"] {ENVJS} "+msg);
    };
    $env.debug  = function(){};
    $env.info   = function(){};
    $env.warn   = function(){};
...
__extend__(DOMDocument.prototype, {	
    ...
    loadXML : function(xmlStr) {
        ...
        try {
        	...
        	//doc = $env.parseHTML(xmlStr+"");
        	var dataReader = new java.io.StringReader(new java.lang.String(xmlStr));
        	var map = new java.util.HashMap();
        	var saxp = new Packages.org.ccil.cowan.tagsoup.jaxp.SAXParserImpl.newInstance(map);
        	var xmlReader = saxp.getXMLReader();
        	var reader =  new java.io.BufferedReader(dataReader);
        	var inputSource = new Packages.org.xml.sax.InputSource(reader);
        	var saxSource = new Packages.javax.xml.transform.sax.SAXSource(xmlReader, inputSource);
        	var domResult = new Packages.javax.xml.transform.dom.DOMResult();
        	var transformer = Packages.javax.xml.transform.TransformerFactory.newInstance().newTransformer();
        	transformer.transform(saxSource, domResult);
        	doc = new HTMLDocument(domResult.getNode());
        	$log("this.implementation: "+this.implementation.constructor);
        	$log("myname: "+doc.nodeName);
